...
In diesem Kapitel werden die vier Sammeltypen zunächst kurz vorgestellt. Anschließend wird die Arbeitsweise insbesondere mit Listen ausführlich erläutert.
...
**Dieses Kapitel leidet an den Ausnahmen: geteilter Speicherbereich (betrifft Listen und Dictionaries), Zugriffsoperator nicht für Mengen definiert, Methode `.copy()` nicht für Tupel definiert.**
  ...
  - Mengen als letztes vorstellen, da diese am Unterschiedlichsten sind.

  - Sonderthemen später behandeln

    - Arbeiten mit Listen

    - Unterschiede der übrigen Sammeltypen zu Listen

## Listen
Wie alle Typen in Python, werden Listen durch Zuweisung erstellt. Bei der Definition einer Liste, werden die Elemente durch `[]` eingeklammert und mit Kommata `,` getrennt. Listen können mit dem `+`-Operator verkettet werden. `*` verkettet eine Liste n-mal.
...
2. 4-Tage-Woche: Verwenden Sie Listenmethoden, um den Freitag aus der Liste 'wochentage' zu entfernen und der Liste 'wochenende' hinzuzufügen.
...
| Speicherbereich bei Zuweisung geteilt          | ja     | Aufhebung bei Neuzuweisung  | Aufhebung bei Neuzuweisung   | ja         |
| Methode .copy() definiert                      | ja     | nein  | ja     | ja         |
| Slice-Operator anwendbar                       | ja     | ja    | nein   | ja (nach Schlüssel) |

&nbsp;

Die Sammeltypen können ineinander umgewandelt werden. **wohin?!**
...
## Funktionen
Einige in Python enthaltene Funktionen lassen sich auch auf Sammeltypen anwenden:

* `len()`: liefert die Länge einer Liste, bzw. Sammeltyps, zurück
* `min()`, `max()`, `sum()`: gibt das Minimum, Maximum bzw. die Summe einer numerischen Liste zurück

## Operationen: Verwendung von Schleifen
Um arithmetische und logische Operatoren auf die in einem Sammeltyp gespeicherten Elemente anzuwenden, wird eine for-Schleife verwendet. Im folgenden Beispiel wird eine Liste 'zahlen' durchlaufen, die darin gespeicherten Zahlen quadriert und das jeweilige Ergebnis an die Liste 'quadratzahlen' angehängt. xi
...
In Python kommt Listen die größte Bedeutung zu, da diese vielseitig eingesetzt werden könnnen. 

Vergleichsoperation zeihen, dann Aufgabe Bubblesort (mit Pseudocode aus w-pseudocode, kann man die grafische Lösung einbinden?)
...
if len(meine_liste) > 1: 
    ...
    print("Liste sortiert:", *meine_liste) # * unterdrückt die Kommas zwischen den Listenelementen
